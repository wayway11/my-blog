...
### Requirement: Gitee repository hosting

项目代码 SHALL 托管在 Gitee（码云）仓库中，博客文章（Markdown 文件）作为仓库内容的一部分进行版本控制。Gitee 国内访问稳定，解决了 GitHub 在国内 push/pull 不稳定的问题。

#### Scenario: Author pushes new article to Gitee

- **WHEN** 作者写完文章后执行 `git push` 将变更推送到 Gitee
- **THEN** 推送过程快速完成，仓库中 `src/content/blog/` 目录包含新文章

#### Scenario: EdgeOne Pages reads from Gitee

- **WHEN** EdgeOne Pages 从 Gitee 仓库读取代码
- **THEN** 国内网络下连接稳定，不会出现超时或读取失败

### Requirement: EdgeOne Pages project creation

项目 SHALL 在腾讯云 EdgeOne Pages 控制台创建，连接到 Gitee 仓库，实现 git push 自动触发构建和部署。

#### Scenario: Push triggers auto deploy

- **WHEN** 作者推送代码到 Gitee 仓库主分支
